...
BEGIN
	...
	SELECT @grade = @correct * 1.0 / @total
	RETURN @grade
END;
GO
...
RETURNS	TABLE AS
RETURN
	SELECT C.Ma_cau_hoi, (SELECT dbo.i7d (C.Ma_cau_hoi, @LT)) AS Dno,(SELECT dbo.i7s (C.Ma_cau_hoi, @LT)) AS Sno
...
RETURNS	TABLE AS
RETURN
	SELECT Ma_mon_hoc, STT, (Dno * 1.0 / Tno) AS Ratio
...
CREATE OR ALTER FUNCTION i11 (@MH AS varchar(10), @LT AS INT)
RETURNS	TABLE AS
RETURN
	SELECT D.Trich_Lan_thi, DT.Ma_cau_hoi, (SELECT dbo.i7s (DT.Ma_cau_hoi,D.Trich_Lan_thi)) AS Sno
	FROM (DE_THI AS D JOIN DE_THI_BAO_GOM_CAU_HOI AS DT ON D.Ma_de_thi = DT.Ma_de_thi)
